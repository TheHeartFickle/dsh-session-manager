window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//
		// 回退条目：按轨迹收集用户输入，功能有且仅有“定位用户输入的位置并重建
		// 分支”。fork 的 atSeq 语义是“向前吸附到首个 turn/end 事件”，因此边界取
		// 该条目之前最近一条用户消息的事件 seq——它必然落在上一回合内部，fork
		// 正好吸附到上一回合结束处，且不依赖 timeline 是否加载完整。
		function collectEntries(chat) {
			...
			for (var i = 0; i < order.length; i++) {
				...
				userInputs.push(node);
			}
			var entries = [];
			for (var j = 1; j < userInputs.length; j++) {
				var current = userInputs[j];
				var previous = userInputs[j - 1];
				entries.push({
					key: current.key,
					boundary: previous.data && previous.data.seq !== undefined ? previous.data.seq : previous.anchorSeq,
					turn: turnOf(current),
					text: textOf(current.data && current.data.content),
					time: current.data && current.data.time,
				});
			}
			entries.reverse();
			return entries;
