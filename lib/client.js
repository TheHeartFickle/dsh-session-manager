window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var rewindZh = {
			...
			confirming: "回退中…",
			errorTitle: "回退失败",
			errorNoPrevious: "找不到这条消息之前的回合，无法回退。",
			errorGeneric: "回退失败，请稍后重试。",
			fileChangesHint: "检测到这些回合中有文件写入。",
			rewindSessionOnly: "仅会话回退",
			rewindWithFiles: "带文件回退",
			gitFallbackWarning: "Git 不可用或缺少该回退点的快照，已改用 best-effort 文件回退：",
			gitSaving: "正在 Git 保存快照…",
		...
		var rewindEn = {
			...
			errorNoPrevious: "No completed turn before this message; cannot rewind.",
			errorGeneric: "Rewind failed. Please try again.",
			fileChangesHint: "File writes were detected in these turns.",
			rewindSessionOnly: "Session only",
			rewindWithFiles: "Rewind with files",
			gitFallbackWarning: "Git is unavailable or no snapshot exists for this rewind point; fell back to best-effort file rollback: ",
			gitSaving: "Saving Git snapshot…",
			shortcut: "Ctrl/Cmd+Shift+Z",
		};
		...
		function detectFileChanges(chat, fromTurn) {
			...
			for (var i = 0; i < order.length; i++) {
				...
				if (turn === undefined || turn < fromTurn) continue;
				var root = node.data && node.data.root;
				var name = root && (root.name || (root.call && root.call.name));
				if (name === "write" || name === "edit") return true;
			}
			return false;
		}

		function turnMaps(chat) {
			var turnStarts = new Map();
			var turnEnds = new Map();
			var timeline = chat && chat.timeline;
			if (timeline && timeline.turns) {
				timeline.turns.forEach(function (turn, num) {
					if (turn && turn.start && Number.isFinite(turn.start.seq)) turnStarts.set(num, turn.start.seq);
					if (turn && turn.end && Number.isFinite(turn.end.seq)) turnEnds.set(num, turn.end.seq);
				});
			}
			var legacyEnds = chat && chat.legacy && chat.legacy.turnEnds;
			if (legacyEnds) {
				legacyEnds.forEach(function (seq, turn) {
					if (!turnEnds.has(turn)) turnEnds.set(turn, seq);
				});
			}
			return { turnStarts: turnStarts, turnEnds: turnEnds };
		}

		function turnBoundary(turn, maps) {
			// 优先取该回合之前最近的已关闭回合边界：这通常正好是 turn/end，
			// 也是 git 快照里一定存在的回退点。
			for (var t = turn - 1; t >= 1; t--) {
				if (maps.turnEnds.has(t)) return maps.turnEnds.get(t);
			}
			// 没有可用前序边界时，再退回“该回合 turn/start 之前”这一通用安全点，
			// 覆盖截断/未来日志形态。
			var start = maps.turnStarts.get(turn);
			if (typeof start === "number" && start > 0) return start - 1;
			return undefined;
		}
		...
		function collectEntries(chat) {
			...
			for (var i = 0; i < order.length; i++) {
				...
				var turn = turnOf(node);
				if (turn === undefined || turn <= 1) continue;
				var boundary = turnBoundary(turn, maps);
				if (boundary === undefined) continue;
				var content = node.data && node.data.content;
				var text = textOf(content);
				entries.push({
					key: node.key,
					seq: node.data && node.data.seq !== undefined ? node.data.seq : node.anchorSeq,
					turn: turn,
					boundary: boundary,
					text: text,
					time: node.data && node.data.time,
					hasFileChanges: detectFileChanges(chat, turn),
				});
			}
			entries.reverse();
			return entries;
		...
		function RewindModal(props) {
			...
			function doRewind(mode) {
				...
				var prevEnd = selected.boundary;
				if (prevEnd === undefined) {
					setError(props.t("errorNoPrevious"));
					return;
				}
				setBusy(true);
				setError(null);
				...
				if (mode === "files") {
					var snapshotAtSeq = (props.turnStarts && props.turnStarts.get(selected.turn)) || prevEnd;
					rollback = fetch("/api/session-manager/rewind/rollback-files", {
						method: "POST",
						headers: { "content-type": "application/json" },
						body: JSON.stringify({
							sessionId: props.sessionId,
							atSeq: prevEnd,
							snapshotAtSeq: snapshotAtSeq
						})
						...
						return response.json().catch(function () {
							...
							if (!response.ok || !data.ok) {
								throw new Error((data && data.error) || props.t("errorGeneric"));
							}
							if (data.mode === "best-effort" && data.warning) {
								window.alert(props.t("gitFallbackWarning") + "\n" + data.warning);
							}
							return data;
						});
					});
			...
			var footer;
			if (selected !== null && selected.hasFileChanges) {
				var fileEnabled = props.fileMode === "git" || props.fileMode === "diff";
				var fileButton = fileEnabled ? React.createElement(Button, {
					variant: "primary",
					onClick: function () { doRewind("files"); },
					disabled: busy
				}, busy ? props.t("confirming") : props.t("rewindWithFiles")) : null;
				footer = React.createElement(React.Fragment, null,
					...
					React.createElement(Button, {
						variant: fileEnabled ? "outline" : "primary",
						onClick: function () { doRewind("session"); },
						disabled: busy
					}, busy ? props.t("confirming") : props.t("rewindSessionOnly")),
					fileButton
				);
			} else {
				footer = React.createElement(React.Fragment, null,
			...
			var body = [
				...
				selected !== null && React.createElement("div", { key: "preview", className: "dsh-rewind-preview" },
					...
					selected.hasFileChanges && React.createElement("div", { className: "dsh-rewind-file-hint" }, props.t("fileChangesHint"))
				),
		...
		function RewindHeaderAction(props) {
			...
			var gitSavingState = React.useState(false);
			var gitSaving = gitSavingState[0];
			var setGitSaving = gitSavingState[1];
			var fileModeState = React.useState("git");
			var fileMode = fileModeState[0];
			var setFileMode = fileModeState[1];

			React.useEffect(function () {
				var dead = false;
				fetch("/api/session-manager/config")
					.then(function (res) { return res.json(); })
					.then(function (data) {
						if (dead) return;
						if (data && (data.rewindFileMode === "git" || data.rewindFileMode === "diff" || data.rewindFileMode === "none")) {
							setFileMode(data.rewindFileMode);
						}
					})
					.catch(function () { });
			...
			React.useEffect(function () {
				...
				var es = new EventSource("/api/session-manager/rewind/status?sessionId=" + encodeURIComponent(props.sessionId));
				es.onmessage = function (event) {
					try {
						var data = JSON.parse(event.data);
						if (data.type === "snapshot" && data.sessionId === props.sessionId) {
							setGitSaving(data.phase === "start");
						}
					} catch (_) { }
				};
				es.onerror = function () {
			...
			var chat = typeof props.useChat === "function" ? props.useChat(function (s) { return s; }) : undefined;
			var entries = React.useMemo(function () { return collectEntries(chat); }, [chat]);
			var turnStarts = React.useMemo(function () {
				var map = new Map();
				var timeline = chat && chat.timeline;
				if (timeline && timeline.turns) {
					timeline.turns.forEach(function (turn, num) {
						if (turn && turn.start && Number.isFinite(turn.start.seq)) map.set(num, turn.start.seq);
					});
				}
				return map;
			}, [chat]);
			...
			return React.createElement(React.Fragment, null,
				...
				gitSaving && React.createElement("span", { className: "dsh-rewind-saving", role: "status" }, props.t("gitSaving")),
		...
		var CSS = [
			...
			".dsh-rewind-saving{box-sizing:border-box;display:inline-flex;align-items:center;gap:6px;color:var(--dsw-alias-label-secondary,#c6cbd1);background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08));border:1px solid var(--dsw-alias-border-l1,#3a3f45);border-radius:999px;padding:3px 10px;font-size:12px;line-height:18px;white-space:nowrap}",
			".dsh-rewind-saving::before{content:\"\";width:8px;height:8px;border-radius:50%;background:var(--dsw-alias-state-business-primary,#4d9fff);animation:dsh-rewind-pulse 1s ease-in-out infinite}",
			"@keyframes dsh-rewind-pulse{0%,100%{opacity:.35}50%{opacity:1}}",
			...
			".dsh-rewind-file-hint{box-sizing:border-box;color:var(--dsw-alias-state-warn-label,#e6a23c);background:var(--dsw-alias-state-warn-bg,rgba(230,162,60,.12));border:1px solid var(--dsw-alias-state-warn-border,rgba(230,162,60,.3));border-radius:8px;padding:6px 10px;font-size:12px;line-height:18px}",
