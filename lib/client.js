window.__ModuleLoader__.load({
	id: "@the-heart-fickle/dsh-session-manager-plugin",
	factory: (require) => {
		var module = { exports: {} };
		...
		function detectFileChanges(snapshot, fromTurn) {
		...
		function collectEntries(snapshot) {
			...
			var order = chat.order || [];
			var nodes = chat.nodes;
			var turnEnds = snapshot.turnEnds || (snapshot.chat && snapshot.chat.legacy && snapshot.chat.legacy.turnEnds);
			var entries = [];
			for (var i = 0; i < order.length; i++) {
				var node = nodes.get(order[i]);
				if (!node || node.visibility === "hidden") continue;
				...
				if (!turnEnds || !turnEnds.has(turn)) continue;
				...
				entries.push({
					...
					hasFileChanges: detectFileChanges(snapshot, turn),
				});
			}
			entries.reverse();
			return entries;
		...
		function RewindModal(props) {
			...
			function doRewind(mode) {
				...
				var prevEnd = props.turnEnds.get(selected.turn - 1);
		...
		function RewindHeaderAction(props) {
			...
			var entries = React.useMemo(function () { return collectEntries(snapshot); }, [snapshot]);
			var turnEnds = (snapshot && (snapshot.turnEnds || (snapshot.chat && snapshot.chat.legacy && snapshot.chat.legacy.turnEnds))) || new Map();
			var turnStarts = React.useMemo(function () {
				var map = new Map();
				var timeline = snapshot && snapshot.chat && snapshot.chat.timeline;
