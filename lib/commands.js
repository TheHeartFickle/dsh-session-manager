...
//
// 回退（rewind）不经过本命令面：客户端直接用官方 sessions.fork 重建分支。
//
    ...
    return COMMAND_DEFS.map((def) => ({
      ...
      execute: async function execute() {
        return fail({
          ...
          message: `command ${def.id} has no backend in the standard channel; install through the official channel (cordis.patch.yml) until @dsh-std/adapter-dsh covers session archive semantics`,
        });
      },
    }));
  }
