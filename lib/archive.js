...
async function listArchived(ctx) {
  ...
  try {
    // 0.1.5 的 list() 返回 { header, revision, sizeBytes? } 快照，id 在 header 里
    // （BUGS.md S6：按 h.id 找永远落空，冷会话的 cwd/size/exists 全部失真）。
...
async function deleteSession(ctx, homePath, sessionId) {
  ...
  // 0.1.5 的 list() 元素是 { header } 快照；按 h.id 匹配永远落空会静默跳过
  // 磁盘删除，会话文件残留并在客户端投影为“未分组”（BUGS.md S6）。
  const snapshots = await ctx.sessionPersistence.list();
  const header = (snapshots.find((snapshot) => snapshot.header.id === sessionId) ?? {}).header;
  if (header) {
    const loc = ctx.sessionPersistence.locate(header);
...
export function createArchive(ctx) {
  ...
  // sessionProjectionCache / sessions / dshHomePath）。返回 { status, body }，
  ...
  const deleteAll = async () => {
    const ids = [...ctx.workspaceRegistry.archivedSessionIds];
    return { status: 200, body: await deleteMany(ctx, homePath, ids) };
  };
