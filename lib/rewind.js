...
export function applyRewind(ctx, config) {
  ...
  // 回退边界是否落在“任一回合之外”：向前找到最近的 turn/start 或 turn/end，
  // 只有边界不是位于未关闭的 turn/start 之后才是安全的 fork 起点。
  const isClosedTurnBoundary = (session, seq) => {
    const event = session.events && session.events[seq];
    if (!event || event.seq !== seq) return false;
    for (let i = seq; i >= 0; i--) {
      const current = session.events[i];
      if (!current || current.seq !== i) return false;
      if (current.type === 'turn/start') return false;
      if (current.type === 'turn/end') return true;
    }
    return true;
  };
  ...
  // 文件回退操作（官方服务触点：sessions）。返回 { status, body }，
  // HTTP 状态码语义与旧版 handleRollbackFiles 逐字段一致；由通道层投影
  // （HTTP：lib/index.js；标准命令：lib/commands.js 的 project()）。
  const rewindFiles = async ({ sessionId, atSeq, snapshotAtSeq }) => {
    ...
    if (!sessions) {
      return { status: 503, body: { ok: false, error: 'sessions-unavailable' } };
    }
    ...
    if (!isClosedTurnBoundary(session, atSeq)) {
      return { status: 400, body: { ok: false, error: 'bad-boundary' } };
    }
    if (useSnapshotAtSeq !== atSeq) {
      const snapshotEvent = session.events && session.events[useSnapshotAtSeq];
      if (!snapshotEvent || snapshotEvent.seq !== useSnapshotAtSeq) {
        return { status: 400, body: { ok: false, error: 'bad-snapshot-boundary' } };
      }
    }
    ...
    if (rewindFileMode === 'none') {
      return { status: 409, body: { ok: false, error: 'file-rollback-disabled' } };
    }
    ...
    if (rewindFileMode === 'diff') {
      try {
        ...
        return { status: 200, body: { ok: true, mode: 'diff', ...result } };
      } catch (diffError) {
        return {
          status: 500,
          body: { ok: false, error: diffError instanceof Error ? diffError.message : String(diffError) },
        };
      }
    }
  ...
  return { handleStatus, rewindFiles, getConfig: () => ({ rewindFileMode }) };
}
